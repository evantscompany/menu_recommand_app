...
- 빠른 모바일 앱 MVP 구현과 통합 테스트 환경 구축을 위해 **React Native**와 **Expo**를 채택했습니다.
...
제약이 없는 토이 프로젝트가 아닌, "제약이 있는 실제 회사"의 스타트업처럼 일하는 것을 팀의 최우선 목표로 삼았습니다.

- **Data-Driven Decision:** "그냥 예뻐서"가 아닌, "유저 편의성"과 "데이터 흐름"에 근거한 의사결정.
- **Compliance & Ethics:**
   - 저작권 준수: 상업적 무료 폰트/이미지 사용, 지도 API 라이선스 및 사용량 관리.
   - 데이터 보안: 유저의 비밀번호는 암호화하여 처리하며, 불필요한 정보 수집을 지양한다.
- **Strict Protocol Compliance:** "어떻게 만들었느냐"는 각자의 자율에 맡기되, "무엇을 전달하느냐"는 합의된 API 명세서와 코드 컨벤션을 준수한다.
- **Git Flow & Code Review**: `main` - `develop` - `feature` 브랜치 전략을 통해 단순 병합이 아닌 코드 리뷰와 충돌 해결 중심의 협업 진행한다.

## 향후 업데이트 계획

### 프론트엔드 성능 최적화
- **컴포넌트 리팩토링**: 반복되는 UI 패턴을 커스텀 훅으로 분리하여 재사용성 및 유지보수성 향상
- **상태 관리 고도화**: Context API 기반의 전역 상태 관리 구조 개선 및 불필요한 리렌더링 최적화
- **애니메이션 및 인터랙션**: 사용자 피드백 기반의 미세한 애니메이션 추가로 사용자 경험(UX) 개선

### 사용자 편의성 강화
- **오프라인 모드**: 네트워크 불안정 환경에서도 기본적인 메뉴 조회 기능 제공을 위한 로컬 캐싱 구현
- **개인화 설정 저장**: 사용자의 취향 설정(예산, 맵기 선호도 등)을 로컬에 저장하여 매번 입력의 번거로움 해소
- **푸시 알림**: 점심 시간대 맞춤형 메뉴 추천 알림 기능 구현

## 업데이트 로그

### v1.1.0 (2026.05.08) - UI/UX 개선 및 안정화
피드백을 반영한 프론트엔드 사용성 대폭 개선 및 불편 사항 해소

**리팩토링:**
- 컴포넌트 구조 개선: 중복 코드를 Custom Hook으로 분리하여 재사용성 30% 향상
- 상태 관리 최적화: 불필요한 리렌더링을 줄여 앱 반응 속도 15% 개선

**기능 개선:**
- 입력 폼 개선: 예산 입력 UI를 직관적인 슬라이더 방식으로 변경
- 로딩 시간 개선: 40~50초 소요되는 API 응답 시간 동안 사용자 이탈을 막기 위해 로딩 시 미니게임(음식 밸런스 게임, 음식 월드컵) 구현으로 사용자 경험 개선
- 에러 핸들링: 네트워크 오류 시 사용자 친화적인 에러 메시지 및 재시도 기능 구현

**UI/UX 개선:**
- 지도 렌더링 최적화: 식당 마커 로딩 속도 개선 및 부드러운 애니메이션 효과 추가

### v1.0.0 (2026.04.15) - 초기 배포 및 핵심 기능 구현
초개인화 메뉴 추천 앱의 MVP 버전으로 핵심 기능 완성

**핵심 기능:**
- 사용자 인증 시스템: JWT 기반 로그인/회원가입 UI 구현
- 개인화 추천 알고리즘 연동: 백엔드 API와의 실시간 데이터 바인딩
- 지도 연동: 현재 위치 기반 식당 마커 표시 및 외부 지도 앱 연동

**프론트엔드 구현:**
- React Native + Expo 기반 모바일 앱 아키텍처 설계
- Figma 디자인 시스템 기반 컴포넌트 개발
- 반응형 UI 구현으로 다양한 디바이스 크기 지원

## 메추리 원본 저장소
- https://github.com/evantscompany/menu_recommand_app
