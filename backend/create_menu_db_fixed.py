        ...
        if menu_response.status_code == 200:
            ...
            for i, menu in enumerate(menu_items):
                ...
                sql_content += "INSERT INTO menus (menu_name, category, price, image_url, is_lunch_available, matching_weather, suitable_ground_size, is_quick_meal) VALUES ('{}', '{}', {}, '{}', '{}', '{}', '{}', '{}', '{}');\n".format(
                    ...
                    menu.get("matching_weather", "").replace("'", "''") if menu.get("matching_weather") else "''",
            ...
            # 5. 파일 저장
            with open("insert_menus_fixed.sql", "w", encoding="utf-8") as f:
                f.write(sql_content)
            
            print(f"   ✅ SQL 파일 생성 완료: insert_menus_fixed.sql")
            print(f"   📊 생성된 SQL: {len(menu_items)}개 메뉴 데이터")
            print(f"   📄 파일 저장 위치: {os.path.abspath('insert_menus_fixed.sql')}")
