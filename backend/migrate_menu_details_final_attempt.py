"""
menu_details 테이블 최종 시도 마이그레이션 스크립트
"""
...
def migrate_menu_details_final_attempt():
    """menu_details 테이블 최종 시도 마이그레이션"""
    ...
    try:
        ...
        for row_num, row in enumerate(cursor.fetchall(), 1):
            try:
                ...
                # 문자열이 아닌 값은 따옴표로 처리
                insert_sql = insert_sql.replace("'unknown'", "'unknown'").replace("'None'", "NULL")
...
if __name__ == "__main__":
    print("🚀 menu_details 테이블 최종 시도 마이그레이션 시작")
    print("=" * 60)
    print("⚠️ 이 스크립트는 마지막 시도입니다. 실패 시 대안을 준비하세요.")
    print("")
    
    if migrate_menu_details_final_attempt():
        print("\n🎉 menu_details 마이그레이션 성공 완료!")
        print("🎊 모든 데이터가 TiDB Cloud로 마이그레이션되었습니다!")
    else:
        print("\n💥 menu_details 마이그레이션 실패!")
        print("💡 다음 대안을 고려하세요:")
        print("   1. PyMySQL 버전 다운그레이드")
        print("   2. mysql-connector-python 라이브러리 사용")
        print("   3. 수동 데이터 입력")
